...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	//mem with align = 8
	size_t sz = 37, align = 8;
	void *x = sf_memalign(sz,align);
	sf_block *bp = (sf_block *)((char *)x - sizeof(sf_header));

	cr_assert_not_null(x, "x is NULL!");

	cr_assert((size_t)x % align == 0, "sf_memalign failed to give proper aligned addresses");
	cr_assert((bp->header & ~0x7) == 48, "alloced block size not what was expected!");
	assert_quick_list_block_count(0, 0);
	assert_free_block_count(4008,1);
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	...
	cr_assert((size_t)x % align == 0, "sf_memalign failed to give proper aligned addresses");
	cr_assert((bp->header & ~0x7) >= 56, "alloced block size not what was expected!");
	assert_quick_list_block_count(0, 0);
...
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
	//coalesce both side
	sf_errno = 0;
	size_t sz_w = 400, sz_x = 200, sz_y = 300, sz_z = 300;
	void *w = sf_malloc(sz_w);
	void *x = sf_malloc(sz_x);
	void *y = sf_malloc(sz_y);
	/*void *z = */sf_malloc(sz_z);
	...
	assert_free_block_count(2816, 1);

	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}

Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
	//mem with align = 23 which is unacceptable
	sf_errno = 0;
...
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
	//mem with align = 16, sz 0 which should return NULL;
	sf_errno = 0;
	size_t sz = 41, align = 16, zero_sz = 0;
	sf_malloc(sz);
	void *x = sf_memalign(zero_sz,align);
